...
\begin{tikzpicture}[
  ...
  \draw[to] (editor) to[bend left=15] node[midway, above] {edits}
  ...
  \draw[to] (patroller) to[bend right=25] node[midway, above] {} (judgments);
  \draw[to] (judgments) to[bend right=25] node[midway, above] {} (patroller);
  ...
  \draw[to] (patroller) to[loop, out=-60, in=-120, looseness=5] node[near end, left] {discussion, \\ consensus} (patroller);

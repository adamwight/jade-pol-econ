...
%     \& \node[datastore] (scores) {Machine \\ predictions}; \\
    \node [datastore] (judgments) {Patrolled \\ articles};
      \& \node [process] (patroller) {Patrollers}; \\
  };
...
% \draw[to] (content) to[bend left=25] node[midway, right] {articles, \\ edits} (ai);
  \draw[to] (content) to[bend left=25] node[midway, right] {articles, \\ edits} (patroller);
% \draw[to] (ai) to[bend left=25] node[midway, above] {raw scores} (scores);
% \draw[to] (scores) to[bend left=25] node[midway, left] {scores, \\ context} (editor);
% \draw[to] (scores) to[bend left=25] node[midway, right] {scores, \\ context, \\ justifications} (patroller);
  \draw[to, dashed] (judgments) to[bend left=25] node[midway, left] {patrol tags, \\ available pages} (editor);
  ...
  \node [align=flush center] at ($(patroller)!0.5!(judgments)$) {patrol tags};
...
% \draw[to] (judgments) to[bend right=25] node[midway, right] {false positives, \\ training data} (ai);
  ...
  \draw[to, dashed] (patroller) to[loop, out=-60, in=-120, looseness=8] node[midway, below] {discussion, \\ consensus} (patroller);
